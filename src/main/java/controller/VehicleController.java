...
public class VehicleController {
    ...
    {
        List<Goods> goods = Goods.dao.find("select * from goods where limit > ?", worldTime);
        Goods purpose = null;
        double maxvalue = 0;
    ...
    public static void calculateVehicleStatus(Vehicle vehicle, int addTime, int worldTime) {
        ...
        if(currentTime + addTime < statusTime) {
            ...
            if(status == 2) {
                ...
                Road nowRoad = Road.dao.findById(vehicle.getInt("road_id"));
                nowGoods.delete();
            ...
            if(nextStatus == 0) {
                ...
                Goods purpose = getPurposeGoods(vehicle, worldTime, vehicle.getInt("current_poi"));
                if(purpose == null) {
                    vehicle.set("status", 0);
                    vehicle.set("status_time", randomTime());
                    vehicle.set("current_time", 0);
                    ...
                    Road road1 = RoadController.getRoad(vehicle.getInt("current_poi"), purpose.getInt("st_id"));
                ...
                if(!nowGoods.getBoolean("is_delivered")) {
                    Road road1 = RoadController.getRoad(vehicle.getInt("st_id"), nowGoods.getInt("ed_id"));
                    int road_id = road1.getInt("id");
